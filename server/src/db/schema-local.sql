-- Local (PGlite) schema: simulation data only, no FK to cloud users table
CREATE EXTENSION IF NOT EXISTS pgcrypto;
CREATE SCHEMA IF NOT EXISTS antsim;
SET search_path TO antsim;

-- user_id stored as plain UUID — no cross-DB FK constraint
...
CREATE INDEX IF NOT EXISTS idx_conversations_simulation ON individual_conversations(simulation_id, sim_day);
CREATE INDEX IF NOT EXISTS idx_conversations_individual ON individual_conversations(individual_id);
CREATE INDEX IF NOT EXISTS idx_technologies_tech_id ON technologies(simulation_id, tech_id);
CREATE INDEX IF NOT EXISTS idx_beliefs_simulation ON belief_systems(simulation_id, extinct);
CREATE INDEX IF NOT EXISTS idx_language_records_simulation ON language_records(simulation_id, recorded_day);
CREATE INDEX IF NOT EXISTS idx_individuals_parent ON individuals(parent_1_id);
