// Epigenetics Engine
// Cardinal Rule: epigenetic changes on non-founders are physiological responses to the
// individual's own internal signals — not scripted behaviours. Heritability coefficients
// in inheritEpigenome() propagate these patterns genetically to offspring.
const LOCI = {
...
export function inheritEpigenome(child, p1, p2) {
  if (!p1.epigenome) initializeEpigenome(p1);
  if (!p2.epigenome) initializeEpigenome(p2);
  child.epigenome = {};
  for (const [id, locus] of Object.entries(LOCI)) {
    const m1 = p1.epigenome[id]?.methylation ?? 0.5;
    const m2 = p2.epigenome[id]?.methylation ?? 0.5;
    child.epigenome[id] = {
      methylation: Math.min(
        Math.max(0.5 + ((m1 + m2) / 2 - 0.5) * locus.heritability, 0),
        1
      ),
      last_modified: 0
    };
...
export function updateEpigenome(individual, env, simDay) {
  ...
  const nutrition = individual.satiation ?? 0.5;
  const social = individual.group_id ? 0.6 : 0.2;
  mod(individual, 'HPA_AXIS', stress > 0.7 ? 0.02 : -0.005, simDay);
  mod(individual, 'LEPTIN_RESIST', nutrition < 0.3 ? 0.01 : -0.005, simDay);
  mod(individual, 'OXTR_METHYL', social < 0.3 ? 0.01 : -0.01, simDay);
...
function mod(individual, id, delta, simDay) {
  const locus = LOCI[id];
  if (!locus) return;
  if (!individual.epigenome) initializeEpigenome(individual);
  if (!individual.epigenome[id]) {
  ...
  individual.epigenome[id].last_modified = simDay;
}
...
export function computeEpigeneticAge(individual) {
  if (!individual.epigenome) return individual.age;
  const ca = individual.age / 365;
  return ca * (
    0.8 +
    ((individual.epigenome.HPA_AXIS?.methylation ?? 0.5) +
      (individual.epigenome.LEPTIN_RESIST?.methylation ?? 0.5)) / 2 * 0.4
  );
}
