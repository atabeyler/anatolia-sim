...
describe('processAstronomyTick — celestial events', () => {
  ...
  it('lunar_cycle event fires on day 0 (period 29.5 → day 0 % 30 < 1)', () => {
    const obs = new Set();
    processAstronomyTick([], obs, new Set(), new Set(), 0);
    // Day 0 % 30 === 0 < 1, observability 0.9 — may or may not fire (probabilistic)
    // We just check the function doesn't throw
  });

  it('observation is added to set when event fires', () => {
    const obs = new Set();
    // Day 0 aligns with lunar_cycle period (29.5 → 30 rounded), run enough times to get one
    let fired = false;
  ...
  it('low-curiosity observer cannot unlock knowledge', () => {
    ...
    // curiosity ≤ 0.5 threshold fails the observer filter
    expect(knowledge.size).toBe(0);
  });
  ...
  it('low iq prevents specific knowledge unlock', () => {
    ...
    // iq_min for lunar_tracking is 0.3 — 0.1 falls below
  ...
  it('seasonal_calendar requires calendar tech', () => {
    ...
    // No calendar tech → should NOT unlock seasonal_calendar
