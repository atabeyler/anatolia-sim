...
export default function PerformancePanel() {
  ...
  return (
    ...
    >
      {/* Warp status */}
      {isWarping && (
        <div style={{ background: 'rgba(212,168,56,0.1)', border: '1px solid rgba(212,168,56,0.4)', padding: '8px 10px', marginBottom: 8 }}>
          <div style={{ fontSize: 11, color: '#d4a838', letterSpacing: '0.12em', marginBottom: 4 }}>
            ⚡ {t('WARP MOD AKTİF', 'WARP MODE ACTIVE', 'WARP-MODUS AKTIV', 'MODE WARP ACTIF', 'وضع الوارب نشط')}
          </div>
          {warpYear && <div style={{ fontSize: 11, color: '#e0c870' }}>{t('Hedef Yıl', 'Target Year', 'Ziel-Jahr', 'Année Cible', 'السنة المستهدفة')}: {warpYear}</div>}
          {currentYear && <div style={{ fontSize: 11, color: '#e0c870' }}>{t('Mevcut Yıl', 'Current Year', 'Aktuelles Jahr', 'Année Actuelle', 'السنة الحالية')}: {currentYear}</div>}
          {warpPct !== null && (
            <div style={{ marginTop: 6 }}>
              <div style={{ fontSize: 10, color: '#a0c8b0', marginBottom: 2 }}>{warpPct.toFixed(1)}%</div>
      ...
      {m ? (
        <>
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 10, color: '#4f6ef7', letterSpacing: '0.14em', marginBottom: 6 }}>{t('TİCK ZAMANLAMA', 'TICK TIMING', 'TICK-TIMING', 'TIMING TICK', 'توقيت التيك')}</div>
            {[
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 10, color: '#4f6ef7', letterSpacing: '0.14em', marginBottom: 6 }}>{t('MOTOR', 'ENGINE', 'MOTOR', 'MOTEUR', 'المحرك')}</div>
            {[
          ...
          {m.milestones_reached?.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 10, color: '#4f6ef7', letterSpacing: '0.14em', marginBottom: 6 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {m.milestones_reached.map(key => (
                  <span key={key} style={{ fontSize: 9, color: '#d4a838', border: '1px solid rgba(212,168,56,0.3)', padding: '1px 5px', letterSpacing: '0.06em' }}>
                    {key.replace(/_/g, ' ')}
                  </span>
                ))}
              </div>
                ...
                {(() => {
                  ...
                  return [
                    ...
                    <div key={label} style={{ fontSize: 10 }}>
                      <div style={{ color: '#6090a0' }}>{label}</div>
                      <div style={{ color: '#7dd3fc', fontFamily: 'Orbitron, monospace' }}>{value}</div>
                    </div>
                  ));
                })()}
              </div>
