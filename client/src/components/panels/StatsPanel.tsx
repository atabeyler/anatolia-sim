...
export default function StatsPanel() {
  ...
  // Drag state — position of the FAB button (bottom-right anchor by default)
  const [pos, setPos] = useState({ x: 16, y: 16 }); // distance from right/bottom
  ...
  function startDrag(clientX: number, clientY: number) {
    ...
    moved.current = false;
    dragStart.current = { clientX, clientY, posX: pos.x, posY: pos.y };
  }
  ...
  return (
    <>
      ...
      {/* Draggable container — anchored bottom-right via pos */}
      <div style={{ position: 'absolute', bottom: pos.y, right: pos.x, zIndex: 40, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>

        {/* Animated panel — sits above the FAB */}
        <div
        ...
        >
          ...
          <div
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10, cursor: 'grab' }}
          >
            ...
            <button onClick={() => setOpen(false)} style={{ marginLeft: 6, background: 'transparent', border: 'none', color: '#4a6a5a', cursor: 'pointer', lineHeight: 0, padding: 2 }}>
              <X size={12} />
            </button>
          </div>
        ...
        <button
          ...
          title={lang === 'tr' ? 'Nüfus Telemetrisi — sürükle' : 'Population Telemetry — drag'}
        >
