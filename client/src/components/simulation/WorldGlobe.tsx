  ...
  return (
    <>
      {hasFounders && founders.map(({ ind, pos }) => (
        <MarkerDot key={ind.id ?? `founder-${pos.join(',')}`} position={pos} color="#fff176" radius={0.032} />
      ))}
      {hasMales && males.map(({ ind, pos }) => (
        <MarkerDot key={ind.id ?? `male-${pos.join(',')}`} position={pos} color="#90caff" radius={0.027} />
      ))}
      {hasFemales && females.map(({ ind, pos }) => (
        <MarkerDot key={ind.id ?? `female-${pos.join(',')}`} position={pos} color="#ffaacc" radius={0.027} />
      ))}
    </>
  );
}
