...
`updateEpigenome()` modifies methylation levels on non-founder individuals in response to stress, nutrition, and isolation. This is permitted under the cardinal rule because methylation responses are pre-programmed by the genome. The environment triggers a genetically encoded mechanism; it does not inject external behavior. Code touching epigenome outside of `epigeneticsEngine.js` still requires review.
